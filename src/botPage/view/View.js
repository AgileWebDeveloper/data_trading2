...
export default class View {
    ...
    addBindings() {
        ...
        const checkTriggerRun = () => {
            ...
            let min = 0.0;
            let max = 0.0;
            let startCondition = 0.0;
            const currentSymbol = globalObserver.getState('symbol');
        ...
        globalObserver.register('bot.startagain', () => {
            let day = globalObserver.getState('day');
            let hour = globalObserver.getState('hour');
            let minute = globalObserver.getState('minute');
            let second = globalObserver.getState('second');
            const currentTime = new Date();
            let infinite = false;
            if (
                (day == 0 || day == '' || day == undefined) &&
