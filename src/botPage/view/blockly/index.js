...
export const load = (blockStr, dropEvent = {}) => {
    ...
    const str = '<field name="SYMBOL_LIST">';
    const field_str = '</field>';
    const start_index = blockStr.search(str);
    const end_index = blockStr.indexOf(field_str, start_index);
    const temp = blockStr.slice(start_index, end_index);
    const symbol = temp.split('>')[1];
    globalObserver.setState({ symbol });
