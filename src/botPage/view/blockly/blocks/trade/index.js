...
Blockly.JavaScript.trade = block => {
    ...
    const TP = globalObserver.getState('TP');
    const SL = globalObserver.getState('SL');
    const Stake = globalObserver.getState('Stake');
    const str = initialization;

    const str_list1 = str.split(')');
    const str_list2 = str_list1[0].split('(')[1].split(',');
